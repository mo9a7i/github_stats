...
export function GitHubOrg({ org }: GitHubOrgProps) {
    ...
    return (
        <AccordionItem value={org.login}>
            <AccordionTrigger>
                <div className="w-full text-xl flex flex-col md:flex-row md:justify-between font-medium items-start md:items-center md:gap-2">
                    <div className="flex items-center gap-2">
                        <Image src={org.avatar_url} alt={org.login} width={64} height={64} className="rounded-lg shadow-md bg-neutral-800" />
                        <div className="w-full flex bg-yellow-200 justify-between">
                            ...
                            <small className="flex bg-pink-200 text-sm text-gray-500 me-2">{repoCountStr}</small>
                        </div>
                    </div>
                </div>
            </AccordionTrigger>
